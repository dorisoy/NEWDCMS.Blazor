...
using DCMS.Application.Responses.Identity;
...
using DCMS.Client.Infrastructure.Managers.Communication;
using DCMS.Shared.Constants.Storage;

namespace DCMS.Client.Pages.Communication
{
    public partial class Chat
    {
        ...
        private async Task SubmitAsync()
        {
            if (!string.IsNullOrEmpty(CurrentMessage) && !string.IsNullOrEmpty(CId))
            {
                ...
                if (response.Succeeded)
                {
                    ...
                    var userName = $"{user.GetUserRealName()}";
        ...
        private async Task LoadUserChat(string userId)
        {
            ...
            if (response.Succeeded)
            {
                ...
                CId = contact.Id;
                CFullName = $"{contact.UserRealName}";
