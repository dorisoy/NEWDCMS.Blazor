...
namespace DCMS.Client.Pages.Identity
{
    public partial class Roles
    {
        ...
        {
            var Model = await RoleService.GetRolesAsync();
            if (Model.Succeeded)
            {
                _roleList = Model.Data.ToList();
            }
            else
            {
        ...
        private async Task Delete(string id)
        {
            ...
            if (!result.Cancelled)
            {
                var Model = await RoleService.DeleteAsync(id);
                if (Model.Succeeded)
                {
                    await Reset();
        ...
        private bool Search(RoleModel role)
        {
