...
namespace DCMS.Client.Pages.Utilities
{
    public partial class AuditTrails
    {
        ...
        private bool Search(AuditModel Model)
        {
            var result = false;
            ...
            if (!result)
            {
                if (Model.TableName?.Contains(_searchString, StringComparison.OrdinalIgnoreCase) == true)
                {
                    result = true;
                }
                if (_searchInOldValues &&
                    Model.OldValues?.Contains(_searchString, StringComparison.OrdinalIgnoreCase) == true)
                {
                    result = true;
                }
                if (_searchInNewValues &&
                    Model.NewValues?.Contains(_searchString, StringComparison.OrdinalIgnoreCase) == true)
                {
                    result = true;
                }
            }
            ...
            if (_dateRange?.Start != null && Model.DateTime < _dateRange.Start)
            {
                result = false;
            }
            if (_dateRange?.End != null && Model.DateTime > _dateRange.End + new TimeSpan(0,11, 59, 59, 999))
            {
                result = false;
            }
        ...
        {
            var Model = await AuditService.GetCurrentUserTrailsAsync();
            if (Model.Succeeded)
            {
                Trails = Model.Data
        ...
        {
            var Model = await AuditService.DownloadFileAsync(_searchString, _searchInOldValues, _searchInNewValues);
            if (Model.Succeeded)
            {
        ...
        public class RelatedAuditTrail : AuditModel
        {
