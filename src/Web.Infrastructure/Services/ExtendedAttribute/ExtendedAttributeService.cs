...
{
    public class ExtendedAttributeService<TId, TEntityId, TEntity, TExtendedAttribute>
        : IExtendedAttributeService<TId, TEntityId, TEntity, TExtendedAttribute>
            ...
            where TId : IEquatable<TId>
    {
        ...
        public ExtendedAttributeService(HttpClient httpClient)
        {
        ...
        public async Task<IResult<List<GetAllExtendedAttributesModel<TId, TEntityId>>>> GetAllAsync()
        {
            var response = await _httpClient.GetAsync(Routes.ExtendedAttributesEndpoints.GetAll(typeof(TEntity).Name));
            return await response.ToResult<List<GetAllExtendedAttributesModel<TId, TEntityId>>>();
        }

        public async Task<IResult<List<GetAllExtendedAttributesByEntityIdModel<TId, TEntityId>>>> GetAllByEntityIdAsync(TEntityId entityId)
        {
            var route = Routes.ExtendedAttributesEndpoints.GetAllByEntityId(typeof(TEntity).Name, entityId);
            var response = await _httpClient.GetAsync(route);
            return await response.ToResult<List<GetAllExtendedAttributesByEntityIdModel<TId, TEntityId>>>();
        }
