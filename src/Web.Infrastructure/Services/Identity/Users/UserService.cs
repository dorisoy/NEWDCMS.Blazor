...
namespace DCMS.Web.Infrastructure.Services.Identity.Users
{
    public class UserService : IUserService
    {
        ...
        public async Task<IResult<List<UserModel>>> GetAllAsync()
        {
            var response = await _httpClient.GetAsync(Routes.UserEndpoints.GetAll);
            return await response.ToResult<List<UserModel>>();
        }

        public async Task<IResult<UserModel>> GetAsync(string userId)
        {
            var response = await _httpClient.GetAsync(Routes.UserEndpoints.Get(userId));
            return await response.ToResult<UserModel>();
        }
        ...
        public async Task<IResult<UserRolesModel>> GetRolesAsync(string userId)
        {
            ...
            return await response.ToResult<UserRolesModel>();
        }
